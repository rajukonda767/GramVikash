...
function VoiceAssistantComponent() {
  ...
  // Pre-initialize speech recognition on mount for faster startup
  useEffect(() => {
    if (recognitionInitialized.current) return
  ...
  const startListening = useCallback(() => {
    ...
    // Reuse initialized recognition or create new one
    ...
    recognition.onresult = (event: SpeechRecognitionEvent) => {
      ...
      // Add assistant message and speak immediately
      setMessages((prev) => [...prev, { role: "assistant", text: response }])
      // Speak without delay using direct setTimeout with minimal time
      setTimeout(() => speakText(response), 0)
    }
  ...
  const handleOpen = useCallback(() => {
    ...
    setIsOpen(true)
    // Show welcome message only on the very first open ever
  ...
  const handleClose = useCallback(() => {
    ...
    stopListening()
    stopSpeaking()
    // Keep messages for continuity when reopening!
    // Only clear on full app close, not on panel close
...
export const VoiceAssistant = memo(VoiceAssistantComponent)
